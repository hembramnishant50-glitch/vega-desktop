...
export const SearchSubtitlesModal: React.FC<SearchSubtitlesModalProps> = ({
  ...
  const searchSubtitles = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const sqPath = searchQuery?.startsWith("tt")
        ? `/imdbid-${sq}`
        : `/query-${sq}`;
  ...
  return (
    <div
      className="search-subtitles-modal-overlay"
      onClick={onClose}
      onMouseDown={(e) => e.stopPropagation()}
    >
      <div
        className="search-subtitles-modal"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="modal-header">
          <div>
            <p>Playback tools</p>
            <h2 id="subtitle-search-title">Find subtitles</h2>
          </div>
          <button
            className="close-btn"
            onClick={onClose}
